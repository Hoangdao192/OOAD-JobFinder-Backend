/*   ATTENTION
   If you are using Intellij Idea to view this file, so you might see these error:
       '^' unexpected
       COLLATE expected, got '^'
       ',', <index option> or PARTITION expected, got '^'
       <reference> expected, got '^'
   So don't worry, this because the char '^;'
   I have config the separator like '^;' instead of ';' in application.properties
   because when use ';' as separator, it cause problem with the create procedure statement,
   show don't change anything or if you do, remember to use '^;' as the statement separator
 */
...
/*Procedure for find job*/
drop procedure if exists find_job^;
create procedure find_job(in job_title text, in company_id bigint, in major text, in working_form text, in page_number int, in page_size int)
    begin
        declare _page int;
        declare _page_size int;
        ...
        if (page_number is null  or page_number < 0) then
            ...
            set _page = page_number * _page_size;
        end if;

        if (job_title is not null and job_title != "") then
            select * from job
            where (company_id is null or company_id = -1 or company_id = job.company_user_id) and
                (major is null or major = "" or major = job.major) and
                (working_form is null or working_form = "" or job.working_form = working_form)
              and match (job.job_title) against (job_title in boolean mode)
            limit _page_size offset _page;
        else
            select * from job
            where (company_id is null or company_id = -1 or company_id = job.company_user_id) and
                (major is null or major = "" or major = job.major) and
...
/*Count function*/
drop function if exists find_job_count ^;
create function find_job_count(
    job_title text, company_id bigint, major text, working_form text
) returns bigint
reads sql data
begin
    declare result_number bigint;
    if (job_title is not null and job_title != "") then
        select count(*) into result_number from job
        where (company_id is null or company_id = -1 or company_id = job.company_user_id) and
            (major is null or major = "" or major = job.major) and
            (working_form is null or working_form = "" or job.working_form = working_form)
          and match (job.job_title) against (job_title in boolean mode);
...
/*Job view*/
drop view if exists list_jobs ^;
create view list_jobs as
    select job.*, concat(address.detail_address, ", ", address.ward, ", ", address.district, ", ", address.province) from job
